document.addEventListener('DOMContentLoaded', function() {
    ...
    // Algorithm outputs - straight output only
    const algorithmOutputs = {
        "Linear Search": "Element 8 found at position 3",
        "Binary Search": "Element 8 found at position 4",
        "Bubble Sort": "[1, 2, 5, 8, 9]",
        "Insertion Sort": "[1, 2, 5, 8, 9]",
        "Selection Sort": "[1, 2, 5, 8, 9]",
        "Merge Sort": "[1, 2, 5, 8, 9]",
        "Quick Sort": "[1, 2, 5, 8, 9]",
        "Depth-First Search": "1 → 2 → 4 → 5 → 3 → 6",
        "Breadth-First Search": "1 → 2 → 3 → 4 → 5 → 6",
        "Dijkstra's Algorithm": "A→B: 4, A→C: 2, A→D: 5, A→E: 3, A→F: 9"
    };
    ...
    pseudocodeExamples.forEach((example, index) => {
        const card = document.createElement('div');
        card.className = 'bg-white p-6 rounded-xl shadow-md';
        card.setAttribute('data-title', example.title);
        ...
        card.innerHTML = `
            ...
            <button class="run-btn mt-4 px-4 py-2 bg-secondary-600 text-white rounded-lg hover:bg-secondary-700 transition w-full font-medium flex items-center justify-center gap-2">
    ...
    document.querySelectorAll('.run-btn').forEach((button, index) => {
        button.addEventListener('click', function() {
            const title = pseudocodeExamples[index].title;
            const output = algorithmOutputs[title] || "Output not available";
                ...
                <div class="bg-white rounded-xl shadow-2xl p-6 max-w-md w-full mx-4 transform transition-all">
                    <div class="flex items-center justify-between mb-4">
                        <div class="flex items-center gap-2">
                            <i data-feather="terminal" class="w-5 h-5 text-secondary-600"></i>
                    ...
                    <div class="bg-gray-50 p-4 rounded-lg border-2 border-secondary-500">
                        <pre class="text-sm text-gray-700 whitespace-pre-wrap">${output}</pre>
                    </div>
                </div>
            `;
